    ...
    <footer className="border-t border-white/5 py-12 px-6 mt-20">
      <div className="max-w-6xl mx-auto">
        <div className="grid sm:grid-cols-3 gap-8 mb-10">
          ...
          <div>
            <h4 className="text-white font-semibold text-sm mb-3">Why ConvertRX?</h4>
            <ul className="space-y-2 text-sm text-gray-500">
              {([["shield","100% Private — files never leave your browser"],["globe","Works on any device"],["zap","Instant conversion, no waiting"]] as const).map(([type, t], i)=>(
                <li key={i} className="flex items-start gap-2">
                  ...
                  {type === "globe" && <Globe size={14} className="mt-0.5 shrink-0" style={{ color: "#5ca823" }} />}
                  {type === "zap" && <Zap size={14} className="mt-0.5 shrink-0" style={{ color: "#5ca823" }} />}
                  {t}
                </li>
              ))}
            </ul>
