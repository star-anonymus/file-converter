    ...
    <>
      ...
      <main className="pt-24 pb-16 px-6">
        <div className="max-w-4xl mx-auto">
            ...
            <div className="flex gap-2">
              {(["A4", "Letter", "Fit"] as const).map((s) => (
                <button key={s} onClick={() => setPageSize(s)}
                  className={`flex-1 py-2.5 rounded-xl text-sm font-bold transition-all duration-200 border ${pageSize === s ? "bg-emerald-600 border-emerald-500 text-white" : "border-white/10 text-gray-400 hover:border-white/20 hover:text-white"}`}>
          ...
          {/* Dropzone */}
          <div {...getRootProps()} className={`rounded-2xl border-2 border-dashed p-10 text-center cursor-pointer transition-all duration-300 mb-5 ${isDragActive ? "dropzone-active border-emerald-500" : "border-white/10 hover:border-emerald-500/50"}`}>
            <input {...getInputProps()} />
          ...
          <div className="flex flex-wrap gap-3">
            <button onClick={handleConvert} disabled={converting || !images.length}
              className="flex-1 sm:flex-none inline-flex items-center justify-center gap-2 px-8 py-3.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 disabled:opacity-50 disabled:cursor-not-allowed text-white font-semibold transition-all duration-200 shadow-lg shadow-emerald-600/20">
              {converting ? <RefreshCw size={18} className="animate-spin-slow" /> : <FilePlus2 size={18} />}
              {converting ? "Creating PDF..." : "Create PDF"}
            </button>
            {pdfUrl && (
              <a href={pdfUrl} download="images.pdf" className="inline-flex items-center gap-2 px-6 py-3.5 rounded-xl border border-green-500/30 bg-green-500/10 text-green-300 font-semibold hover:bg-green-500/20 transition-all duration-200">
